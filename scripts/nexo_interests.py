...
ap.add_argument("--date_column", type=str, default="Date / Time (UTC)")
...
total = 0
for f in files:
    df = pd.read_csv(f, parse_dates=[args.date_column])[
        [args.usd_column, args.date_column, "Type"]]
    ...
    if args.rates:
        df = df.groupby(df[args.date_column].dt.month)[args.usd_column].sum()
